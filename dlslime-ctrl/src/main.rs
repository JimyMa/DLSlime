//! DLSlime control plane: Control plane server for DLSlime microservice governance.
//!
//! DLSlime control plane is stateless and supports multiple scopes sharing the same instance.
//! Scope is determined by clients such as DLSlimeCache, PeerAgent, and future
//! DLSlime services via `DLSLIME_CTRL_SCOPE`.
...
enum Commands {
    /// Run DLSlime control plane server in the foreground.
    Server(ServerArgs),
    /// Start DLSlime control plane server in the background.
    Start(StartArgs),
    /// Show DLSlime control plane background server status.
    Status(StatusArgs),
    /// Stop DLSlime control plane background server.
    Stop(StopArgs),
...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::registry()
        .with(tracing_subscriber::EnvFilter::new(
            std::env::var("DLSLIME_CTRL_RUST_LOG").unwrap_or_else(|_| "info".into()),
        ))
...
async fn run_server(args: ServerArgs) -> anyhow::Result<()> {
    let redis_url = std::env::var("DLSLIME_CTRL_REDIS_URL").unwrap_or(args.redis_url);
        ...
        let mut conn = repo.conn().await.map_err(|e| {
            anyhow::anyhow!(
                "Failed to connect to Redis at {redis_url}. Please start Redis or set \
                 DLSLIME_CTRL_REDIS_URL / --redis-url to a reachable Redis instance. Details: {e}"
            )
        })?;
...
async fn start_server(args: StartArgs) -> anyhow::Result<()> {
    ...
    if let Some(pid) = read_pid()? {
        if is_pid_running(pid) {
            println!("dlslime-ctrl is already running (pid={pid})");
            return Ok(());
        }
        cleanup_runtime()?;
    }
    ...
    if check_health(&target_address, Duration::from_millis(800)).is_ok() {
        println!(
            "dlslime-ctrl appears to be already running at {} (no pid file managed by this CLI)",
    ...
    let mut child = cmd
        .spawn()
        .map_err(|e| anyhow::anyhow!("Failed to start dlslime-ctrl server. Details: {e}"))?;
    ...
    while std::time::Instant::now() < deadline {
        if let Some(status) = child.try_wait()? {
            ...
            anyhow::bail!(
                "dlslime-ctrl failed to start (exit={}). See log: {}",
                status,
                log_path.display()
            );
        }
        if check_health(&target_address, Duration::from_millis(800)).is_ok() {
            ...
            println!();
            println!("dlslime-ctrl started (pid={})", child.id());
    ...
    println!();
    println!(
        "dlslime-ctrl process started (pid={}), but health check timed out",
        child.id()
    );
    println!("address: {server_address}");
    println!("log: {}", log_path.display());
...
fn status_server(args: StatusArgs) -> anyhow::Result<()> {
    ...
    let address = args
        ...
        .unwrap_or_else(|| "http://127.0.0.1:4479".to_string());

    let pid = read_pid()?;
    let health_ok = check_health(&address, Duration::from_millis(1500)).is_ok();

    match (pid, health_ok) {
        // No pid file but the endpoint answers — server is managed externally
        // (e.g. systemd, docker, kubernetes). That's a perfectly valid state.
        (None, true) => {
            println!("dlslime-ctrl is running (managed externally — no pid file)");
            println!("address: {address}");
            println!("health: ok");
            Ok(())
        }
        (None, false) => {
            println!("dlslime-ctrl is not running");
            println!("address: {address}");
...
fn stop_server(args: StopArgs) -> anyhow::Result<()> {
    ...
    if !is_pid_running(pid) {
        cleanup_runtime()?;
        println!("dlslime-ctrl is not running (stale pid file cleaned)");
        return Ok(());
    }
    ...
    while std::time::Instant::now() < deadline {
        if !is_pid_running(pid) {
            cleanup_runtime()?;
            println!("dlslime-ctrl stopped (pid={pid})");
            return Ok(());
        }
        std::thread::sleep(Duration::from_millis(200));
    }
    ...
    if args.force {
        signal_pid(pid, "KILL")?;
        cleanup_runtime()?;
        println!("dlslime-ctrl killed (pid={pid})");
        return Ok(());
    }

    anyhow::bail!(
        "dlslime-ctrl did not stop within {:.1}s; rerun with --force",
        args.timeout
    )
}

fn runtime_dir() -> PathBuf {
    if let Ok(path) = std::env::var("DLSLIME_CTRL_RUNTIME_DIR") {
        return PathBuf::from(path);
    }
    if let Ok(path) = std::env::var("XDG_RUNTIME_DIR") {
        return PathBuf::from(path).join("dlslime-ctrl");
    }
    PathBuf::from("/tmp/dlslime-ctrl")
}

fn pid_file() -> PathBuf {
    runtime_dir().join("dlslime-ctrl.pid")
}

fn meta_file() -> PathBuf {
    runtime_dir().join("dlslime-ctrl.meta.json")
}

fn default_log_file() -> PathBuf {
    runtime_dir().join("dlslime-ctrl.log")
}
