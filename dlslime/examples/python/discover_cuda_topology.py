...
def _print_summary(topology: dict) -> None:
    ...
    for device in accelerators:
        ...
        print(f"  [{ordinal}] {device.get('uuid', 'unknown')} pci={device.get('pci_bus_id', 'unknown')}")
        print(f"      name={device.get('name', 'unknown')} health={device.get('health', 'unknown')}")
        access = device.get("memory_access", {})
        print(f"      UVA={access.get('unified_addressing', False)} fabric_handle={access.get('cuda_fabric_handle', False)}")
        fabric = device.get("mnnvl")
        if fabric:
            print(f"      MNNVL cluster={fabric.get('cluster_uuid')} clique={fabric.get('clique_id')} membership_ready={fabric.get('membership_ready', False)}")
...
def main() -> None:
    ...
    parser.add_argument("--json", action="store_true", help="print the complete discovery document")
