...
initiator = RDMAEndpoint(num_qp=1, device_name=devices[0], ib_port=1)
target = RDMAEndpoint(num_qp=1, device_name=devices[-1], ib_port=1)
...
rhandler = initiator.register_remote_memory_region(
    mr_key, target.endpoint_info()["mr_info"][mr_key]
)
...
write_slot = initiator.write_with_imm([(lhandler, rhandler, 0, 8, 8)], 1, None)
